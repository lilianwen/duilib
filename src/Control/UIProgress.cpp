...
namespace DuiLib
{
	...
	void CProgressUI::PaintStatusImage(HDC hDC)
	{
		...
		RECT rc = {0};
		if( m_bHorizontal ) {
			rc.right = (LONG)((double)(m_nValue - m_nMin) * (m_rcItem.right - m_rcItem.left) / (m_nMax - m_nMin));
			rc.bottom = m_rcItem.bottom - m_rcItem.top;
		}
		else {
			rc.top = (LONG)((double)(m_rcItem.bottom - m_rcItem.top) * (m_nMax - m_nValue) / (m_nMax - m_nMin));
