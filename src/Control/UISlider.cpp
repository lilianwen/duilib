...
namespace DuiLib
{
	...
	{
		if( m_bHorizontal ) {
			int left = (int)(m_rcItem.left + (double)(m_rcItem.right - m_rcItem.left - m_szThumb.cx) * (m_nValue - m_nMin) / (m_nMax - m_nMin));
			...
			int top = m_rcItem.bottom - m_szThumb.cy - (LONG)((double)(m_rcItem.bottom - m_rcItem.top - m_szThumb.cy) * (m_nValue - m_nMin) / (m_nMax - m_nMin));
			return CDuiRect(left, top, left + m_szThumb.cx, top + m_szThumb.cy); 
		}
	}
	...
	void CSliderUI::DoEvent(TEventUI& event)
	{
		...
		if( event.Type == UIEVENT_BUTTONUP )
		{
			...
			if( m_bHorizontal ) {
				...
				else m_nValue = m_nMin + (int)((m_nMax - m_nMin) * (event.ptMouse.x - m_rcItem.left - m_szThumb.cx / 2.0 ) / (m_rcItem.right - m_rcItem.left - m_szThumb.cx));
			}
			else {
				...
				else m_nValue = m_nMin + (int)((m_nMax - m_nMin) * (m_rcItem.bottom - event.ptMouse.y - m_szThumb.cy / 2.0 ) / (m_rcItem.bottom - m_rcItem.top - m_szThumb.cy));
			}
