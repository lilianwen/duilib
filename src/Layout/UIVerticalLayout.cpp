...
namespace DuiLib
{
	...
	void CVerticalLayoutUI::SetPos(RECT rc)
	{
		...
		for( int it1 = 0; it1 < m_items.GetSize(); it1++ ) {
			...
			if( sz.cy == 0 || pControl->GetYPercent()) {
				nAdjustables++;
                if (pControl->GetYPercent()) {
                    nPercentAdjustables++;
                    totalPercent += pControl->GetYPercent();
                }
			}
			else {
		...
		int cyNeeded = 0;
		int cyExpand = 0;
        int cyNotFixed = MAX(0, szAvailable.cy - cyFixed); // 除掉固定尺寸后剩余的
        int cyPercent = totalPercent*cyNotFixed/100;    // 可变尺寸 先按百分比划分 分完后 再均分给没写百分比又没写height的控件
        int cyAvg = cyNotFixed-cyPercent;
		if( nAdjustables-nPercentAdjustables > 0 ) 
            cyExpand = cyAvg / (nAdjustables-nPercentAdjustables);
		...
		for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
			...
			if( sz.cy == 0 || pControl->GetYPercent()) {
				iAdjustable++;
                if (pControl->GetYPercent()) {
                    sz.cy = (cyNotFixed * pControl->GetYPercent() / 100);
                }    
                else 
                    sz.cy = cyExpand;
