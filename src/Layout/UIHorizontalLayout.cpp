...
namespace DuiLib
{
	...
	void CHorizontalLayoutUI::SetPos(RECT rc)
	{
		...
		for( int it1 = 0; it1 < m_items.GetSize(); it1++ ) {
			...
			if( sz.cx == 0 || pControl->GetXPercent()) {
				nAdjustables++;
                if (pControl->GetXPercent()) {
                    nPercentAdjustables++;
                    totalPercent += pControl->GetXPercent();
                }
			}
			else {
		...
		int cxExpand = 0;
        int cxNeeded = 0;
        int cxNotFixed = MAX(0, szAvailable.cx - cxFixed); // 除掉固定尺寸后剩余的
        int cxPercent = totalPercent*cxNotFixed/100;    // 可变尺寸 先按百分比划分 分完后 再均分给没写百分比又没写height的控件
        int cxAvg = cxNotFixed-cxPercent;
        if( nAdjustables-nPercentAdjustables > 0 ) 
            cxExpand = cxAvg / (nAdjustables-nPercentAdjustables);
		...
		for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
			...
			if( sz.cx == 0 || pControl->GetXPercent()) {
				iAdjustable++;
                if (pControl->GetXPercent()) {
                    sz.cx = (cxNotFixed * pControl->GetXPercent() / 100);
                }    
                else 
                    sz.cx = cxExpand;
